...
void DataLevel::dealLevelTarget()
{
	...
	for (auto i = 0; i < length; i++)
	{
		...
		else
		{
			...
			map<TypeLevelTarget, IdAttribute> dic = dicTypeLevelTarget2IdAttribute;
			auto idAttribute = dic.at(type);
...
void DataLevel::dealPassedIncome(const vector<vector<int>> &award)
{
	...
	/*ManagerHandle::getInstance()->notify((int)ID_OBSERVER::SCENE_MAIN, TYPE_OBSERVER_SCENE_MAIN::UPDATE_LAYOUT_TOP);*/
}
