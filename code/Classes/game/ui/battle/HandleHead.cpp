...
bool HandleHead::init()
{
	...
	do
	{
		/*idObserverSet((int)ID_OBSERVER::HANDLE_HEAD);*/
...
void HandleHead::switchNodeHeadTo(const bool &isMst, const int &indexTo)
{
	...
	if (!isSuccess)
	{
		/*ManagerHandle::getInstance()->notify((int)ID_OBSERVER::HANDLE_ENTITY, TYPE_OBSERVER_HANDLE_ENTITY::DEAL_ROUND_OVER, true);//处理回合结束*/
		return;
	}
	auto speed = 244.0f;
