...
void NodeCircleLine::onTouchMoved(const Vec2 &postionTouchMove)
{
	...
	{
		/*ManagerHandle::getInstance()->notify((int)ID_OBSERVER::LAYER_GRID_SELECT, TYPE_OBSERVER_LAYER_GRID_SELECT::RUN_MAID_GRID_SCALE_SMALL, _indexCurrent, nullptr);*/
		...
		if (isGridMoveOut)
		{
			...
			else
			{
				...
				/*ManagerHandle::getInstance()->notify((int)ID_OBSERVER::HANDLE_GRID, TYPE_OBSERVER_HANDLE_GRID::RUN_ACTION_GRID_SELECTED_MOVE_FROM_SELECT, indexGridSelected, _indexLast);//调用格子飞行特效*/
			}
		}
	}
	else//不在当前格子内
	{
		...
		if (isGridMoveOver)
		{
			...
			{
				/*ManagerHandle::getInstance()->notify((int)ID_OBSERVER::LAYER_GRID_SELECT, TYPE_OBSERVER_LAYER_GRID_SELECT::RUN_MAID_GRID_MOVE_FROM_ACTION_ONE_BACK, indexGridMoveOver);//调用格子飞行特效*/
...
void NodeCircleLine::onTouchEndedCanceled()
{
	...
	if (isVecDataGridMaidSelectedFull && _indexCurrent == -1)
	{
		...
		/*managerUI->notify((int)ID_OBSERVER::LAYER_BATTLE, TYPE_OBSERVER_LAYER_BATTLE::HIDE_LAYER_GRID_SELECT);
		managerUI->notify((int)ID_OBSERVER::HANDLE_HEAD, TYPE_OBSERVER_HANDLE_HEAD::SWITCH_LAYOUT_HEAD_MAID_TOUCHABLE, false);*/
	}
	else
	{
		/*ManagerHandle::getInstance()->notify((int)ID_OBSERVER::LAYER_GRID_SELECT, TYPE_OBSERVER_LAYER_GRID_SELECT::RUN_MAID_GRID_MOVE_FROM_ACTION_ALL_BACK);//调用格子飞行特效*/
