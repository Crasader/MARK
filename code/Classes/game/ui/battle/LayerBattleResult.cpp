...
void LayerBattleResult::runAppearAction(const function<void()> &func /*= nullptr*/)
{
	...
	{
		vector<Vec2> vec = vecMoveDistanceSpriteMaid;
		spriteMaid0->runAction(MoveBy::create(duration, vec[0]));
	});
	vecAction.pushBack(actionCallFunc);
	actionDelay = DelayTime::create(duration + 1.0f / 60.0f);
	vecAction.pushBack(actionDelay);
	...
	{
		vector<Vec2> vec = vecMoveDistanceSpriteMaid;
		spriteMaid1->runAction(MoveBy::create(duration, vec[1]));
		spriteMaid2->runAction(MoveBy::create(duration, vec[2]));
	});
	vecAction.pushBack(actionCallFunc);
	actionDelay = DelayTime::create(duration + 1.0f / 60.0f);
	vecAction.pushBack(actionDelay);
...
void LayerBattleResult::updateSpriteMaid()
{
	...
	for (auto i = 0; i < ENTITY_BATTLE_MAX; i++)
	{
		...
		vector<Vec2> vec = vecMoveDistanceSpriteMaid;
		sprite->setPosition(sprite->getPosition() - vec[i]);
...
void LayerBattleResult::onTouchContinue(Ref *ref, Widget::TouchEventType type)
{
	if (type == Widget::TouchEventType::ENDED)
	{
		...
		if (isGradeUp)
		{
			/*ManagerHandle::getInstance()->notify((int)ID_OBSERVER::LAYER_BATTLE, TYPE_OBSERVER_LAYER_BATTLE::SHOW_LAYER_GRADE_UP);*/
			removeFromParent();
		}
		else
		{
			/*ManagerHandle::getInstance()->notify((int)ID_OBSERVER::SCENE_MAIN, TYPE_OBSERVER_SCENE_MAIN::SWITCH_LAYER, TYPE_OBSERVER_SCENE_MAIN::SHOW_LEVELS);*/
		}
	}
}
...
void LayerBattleResult::onTouchBtnTo(Ref *ref, Widget::TouchEventType type)
{
	if (type == Widget::TouchEventType::ENDED)
	{
		...
		/*ManagerHandle::getInstance()->notify((int)ID_OBSERVER::SCENE_MAIN, TYPE_OBSERVER_SCENE_MAIN::SWITCH_LAYER, typeShow);*/
	}
}
